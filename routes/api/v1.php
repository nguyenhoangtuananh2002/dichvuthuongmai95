...
Route::middleware('auth:sanctum')->group(function(){
    ...
    // Driver Ratings
    Route::controller(App\Api\V1\Http\Controllers\DriverRatings\DriverRatingsController::class)
        ->prefix('/driver-ratings')
        ->as('driver_ratings.')
        ->group(function () {
            Route::get('/', 'index')->name('index');  // List all ratings
            Route::post('/store', 'store')->name('store');  // Submit a new rating
            Route::get('/show/{driverRating}', 'show')->name('show');  // Show a specific rating
            Route::put('/update/{driverRating}', 'update')->name('update');  // Update a specific rating
            Route::delete('/delete/{driverRating}', 'delete')->name('delete');  // Delete a specific rating
        });
});
...
Route::controller(App\Api\V1\Http\Controllers\Auth\EmployeeAuthController::class)
    ->group(function () {
        Route::middleware('auth:sanctum')->prefix('/employee')->as('employee.')->group(function() {
            Route::get('/show', 'show')->name('show');  // Now accessible via /employee/show
            Route::put('/update', 'update')->name('update');
            Route::put('/update-password', 'updatePassword')->name('update_password');
        });
        Route::post('/employee/register', 'register')->name('register');
